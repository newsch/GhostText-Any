...
pub fn ws_thread<S: websocket::stream::Stream>(mut client: websocket::client::sync::Client<S>) {
    ...
    let message: ws_messages::GetTextFromComponent = match message {
        OwnedMessage::Text(raw_json) => serde_json::from_str(&raw_json).unwrap(),
        _ => panic!(),
    };

    info!(
        "Received request with url={} and title={:?}",
    ...
    client
        .send_message(&Message::text(
            serde_json::to_string(&ws_messages::SetTextInComponent {
                text: &resulting_text,
            })
            .unwrap(),
        ))
    ...
    client
        .send_message(&Message::ping("".to_string().into_bytes()))
        .unwrap();
    let message = client.recv_message().unwrap();
